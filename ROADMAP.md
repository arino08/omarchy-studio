...
- [~] E1 Route every write path through `Pipeline` — **found by 0.2.8**: drift → pre → write → reload → **verify → rollback** was used by exactly one feature (`rice::import`); every everyday path saved-and-reloaded directly, snapshotting before/after but never running `hyprctl configerrors` and never auto-reverting a config that breaks Hyprland.
  - [x] **looknfeel** (first module, and the pattern for the rest): new `LookFeel::plan` returns `FileEdit`s instead of writing, `apply` builds an `ApplyPlan` and runs the pipeline. Callers (CLI `apply_looknfeel`, TUI save + reset) stopped taking their own pre/post snapshots — the pipeline owns them and doing both doubled every timeline entry. Verification is **capability-gated**: where `hyprctl configerrors` isn't usable the plan carries no check, because the pipeline would read a failed *probe* as a failed verification and roll back a good change. `rollback-e2e.sh` now drives the injected-configerrors case; verified it fails when the verification is removed
  - [x] **engine fix, surfaced by the above**: a snapshot can't record "this file was absent", so a rejected *first-ever* apply left the file on disk while reporting a successful rollback. `Pipeline::apply` now tracks files it creates and deletes them when rolling back. Unit-tested + covered in the e2e
  - [x] **keybinds**: `plan_overrides` returns a `FileEdit` (`None` when the block already says exactly this, so a no-op apply doesn't snapshot or reload). The store threads through `install_marked`/`remove_marked` and nova's `install_keybind`/`remove_keybind`, so the theme and launcher binds roll back too. The capability gate moved to `engine::hypr_verification` — every Hyprland-reloading module needs the same rule. e2e asserts a rejected bind is reverted *and* the bind made before it survives
  - [x] **animations**: smallest of the set; re-picking the current preset now returns early instead of snapshotting and reloading. Failed-apply reporting folded into one `report_apply_error` helper (animations was dumping a raw `VerifyFailed { .. }` at the user)
  - [x] **waybar**: `apply_watched` hand-rolled save → restart → settle → check → restore, which is exactly `ReloadStep::Restart` + `Verification::ProcessAlive`. Now builds a plan, so a bar-killing edit is reverted by the shared machinery *and* — unlike before — snapshotted, so it survives in the timeline. The one rule the pipeline can't express is kept explicitly: **never spawn a bar the user isn't running** (Quickshell shells), so when Waybar is down the plan carries no reload and no verification
  - [ ] remaining: **mako** (writes through a `.tpl` materializer and reloads via `omarchy-theme-refresh` + `makoctl`, so it needs `ReloadStep::ThemeRefresh`/`MakoReload` and has no `configerrors` analogue to verify with), **monitors** (writes from `monitor_write` in the CLI rather than the core module, so the plan has to move into `modules::monitors` first), **tweaks** (a trait registry whose `set` returns `Vec<PathBuf>` and where some tweaks are filesystem-only, e.g. screenshot dirs — needs a way to express non-file effects before it fits a `FileEdit` plan). Each is less mechanical than the four above; extend `rollback-e2e.sh` per path
